...
func V2ConfigFromURLParams(ctx context.Context, q url.Values) (awsv2.Config, error) {
	...
	for param, values := range q {
		...
		case "awssdk", "usePathStyle", "disableHTTPS":
			// ignore, should be handled before this
		default:
...
// V2ConfigFromURLParams returns an options for aws config
//
//	for AWS SDK v2 initialized based on the URL  parameters in q.
//
...
//   - profile: The shared config profile to use; sets SharedConfigProfile.
//   - endpoint: The AWS service endpoint to send HTTP request.
func V2OptionsFromURLParams(q url.Values) ([]func(*s3v2.Options), error) {
	...
	for param, values := range q {
		...
		case "awssdk", "region", "endpoint", "profile", "dualstack", "fips":
			// ignore, should be handled before this
